...
export default function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      } else if (errorStr.includes("api_key_invalid") || errorStr.includes("invalid api key") || errorStr.includes("authentication")) {
        errorMessage = "আপনার Groq API Key-টি সঠিক নয়। দয়া করে Vercel Settings থেকে সঠিক কি সেট করুন।";
    ...
    <div className="flex h-screen bg-[#F8FAFC] text-[#1E293B] font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative min-w-0 bg-white lg:rounded-l-[40px] shadow-2xl overflow-hidden">
        ...
        <header className="h-20 border-b border-slate-100 flex items-center justify-between px-4 sm:px-6 lg:px-10 bg-white/80 backdrop-blur-xl sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-2xl bg-emerald-50 border border-emerald-100 text-[11px] font-black text-emerald-600 uppercase tracking-widest shadow-sm">
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto px-6 py-10 lg:px-0 scroll-smooth">
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-10">
                <motion.div
                  ...
                  className="w-24 h-24 bg-emerald-600 rounded-[32px] flex items-center justify-center shadow-2xl shadow-emerald-200 relative group"
                >
                  <Terminal className="w-12 h-12 text-white group-hover:scale-110 transition-transform" />
                  <div className="absolute -top-2 -right-2 w-8 h-8 bg-slate-900 rounded-full flex items-center justify-center border-4 border-white">
              ...
              messages.map((msg, idx) => (
                ...
                >
                  <div className={cn(
                    "w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 mt-1 shadow-lg shadow-slate-100",
                  ...
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      msg.role === "user" 
                        ? "message-bubble-user px-6 py-4" 
                        : "message-bubble-model p-6"
                    )}>
                      {msg.role === "user" ? (
